...
export const PollCard = ({ poll, user, onVote, onDelete }) => {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`glass rounded-lg p-4 md:p-6 space-y-6 relative overflow-hidden group border-white/5 ${isExpired ? "bg-slate-950/40" : "bg-slate-900/40"}`}
    >
      <div
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              />
              <span
                className={`text-[11px] font-medium ${isExpired ? "text-primary-400" : poll.hasVoted ? "text-emerald-400" : "text-primary-400"}`}
              >
                {isExpired
                  ? "Consensus Reached"
                  : poll.hasVoted
          ...
          <h3 className="text-md font-medium text-white max-w-2xl">
            {poll.question}
          </h3>
        </div>
        ...
        {poll.options?.map((opt) => {
          ...
          return (
            <button
              ...
              className={`
        relative w-full py-2 px-4 rounded-lg overflow-hidden
        border text-left transition-all
        ${hasVoted ? "cursor-default border-white/5" : "border-white/10 hover:border-primary-500/40"}
        ${isSelected ? "bg-primary-500/15 border-primary-500" : ""}
        ${isWinner ? "border-primary-500" : ""}
      `}
            >
              <div
                className={`absolute inset-y-0 left-0 transition-all duration-700
          ${hasVoted ? "bg-white/5" : "opacity-0"}
          ${isWinner ? "bg-primary-600/25" : ""}
        `}
                style={{
              ...
              <div className="relative flex justify-between items-center">
                <div className="flex items-center gap-2">
                  ...
                  {isSelected && (
                    <span className="text-[9px] px-1.5 py-0.5 rounded-sm bg-primary-500 text-white">
                      YOUR VOTE
                    </span>
                  )}
                </div>
                ...
                {(hasVoted || isExpired) && (
                  <span
                    className={`text-[12px] font-medium ${isWinner ? "text-primary-400" : "text-slate-400"}`}
                  >
                    {opt.percentage}%
                  </span>
                )}
