...
export const CampaignCard = ({ campaign, onSupported }) => {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="glass rounded-lg p-4 md:p-6 flex flex-col md:flex-row gap-6 hover:border-primary-500/20 transition-all group border-white/5 shadow-2xl bg-slate-900/40 relative overflow-hidden"
    >
      <div className="absolute top-0 right-0 w-64 h-64 bg-primary-600/3 blur-3xl -mr-32 -mt-32 transition-colors duration-1000 group-hover:bg-primary-500/10" />
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  />
                  <span
                    className={`text-[10px] ${isSupported ? "text-emerald-400" : "text-rose-400"}`}
                  >
                    {isSupported ? "SUPPORT RECORDED" : "OPERATIONAL PULSE"}
                  </span>
                </div>
                {isOrganizer && (
                  <span className="px-2 py-0.5 bg-amber-500/10 border border-amber-500/20 rounded-lg text-[10px] text-amber-500">
                    Authority
                  </span>
                )}
              </div>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 pt-1 mb-3">
                ...
                <div className="flex items-center gap-1 text-[11px] font-medium text-rose-500/50">
                  <MapPin size={11} className="text-rose-500/50" />
                  {campaign.location || "General Sector"}
                </div>
              </div>

              <h3 className="text-xl font-medium text-white hover:text-primary-300 transition-colors leading-none">
                {campaign.title}
              </h3>
            </div>
            {isOrganizer && (
          ...
          <div className="space-y-3 bg-slate-950/40 p-4 rounded-lg border border-white/5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[12px] font-medium text-slate-400 t">
                  Objective
                </span>
              </div>
              <span className="text-primary-400 font-medium text-[11px]">
                {Math.round(
            ...
            <div className="h-1.5 w-full bg-slate-900 rounded-full overflow-hidden border border-white/5 p-px">
              <motion.div
                initial={{ width: 0 }}
                animate={{
