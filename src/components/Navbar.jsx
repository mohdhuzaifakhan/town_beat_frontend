    ...
    <nav className="sticky top-0 z-50 glass border-b border-white/5 bg-slate-950/40 backdrop-blur-2xl">
      <div className="container mx-auto px-6 h-14 flex items-center justify-between">
        <div className="flex items-center gap-2 group">
          <div className="w-8 h-8 rounded-md bg-primary-600 flex items-center justify-center shadow-md shadow-primary-900/30 group-hover:scale-110 transition-transform">
          ...
          {user ? (
            <div className="flex items-center gap-3 pl-3 border-l border-white/10">
              ...
              >
                <img
                  src={user.avatar}
                  className="w-6 h-6 rounded-lg ring-1 ring-white/20 group-hover:ring-primary-500/50 transition-all"
                />
...
const NavLink = ({ to, icon, label, active, color }) => (
  <Link
    to={to}
    className={`flex items-center gap-2 px-4 py-1.5 rounded-lg transition-all font-bold text-sm group ${
      active
