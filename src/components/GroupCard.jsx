                ...
                <div className="flex gap-5">
                    ...
                    <div className="flex-1 min-w-0 space-y-3">
                        <div className="flex items-start justify-between gap-4">
                            <div className="space-y-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2">
                                    <span className="px-2 py-0.5 rounded-md text-[8px] font-medium   bg-white/5 border border-white/5 text-slate-500">
                                        {group.type}
                                    </span>
                                    {isOwner && (
                                        <span className="px-2 py-0.5 rounded-md text-[8px] font-medium   bg-amber-500/10 border border-amber-500/20 text-amber-500">
                                            Authority
                                        </span>
                                    )}
                                </div>
                        ...
                        <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed font-medium">
                            {group.description || 'Connecting local citizens through shared objectives.'}
                        </p>

                        <div className="flex items-center justify-between pt-2">
                            <div className="flex gap-4">
                                <button
                                    onClick={(e) => { e.stopPropagation(); handleInviteTrigger(); }}
                                    className="text-[12px] font-medium    text-slate-500 hover:text-white transition-colors duration-300 active:scale-95"
                                >
                                    Signal Code
                                </button>
                            </div>
                            <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                                {isOwner ? (
                                    <button
                                        ...
                                        disabled={loading}
                                        className="w-full sm:w-auto px-4 py-2 rounded-lg bg-rose-500/10 hover:bg-rose-500 text-rose-500 hover:text-white border border-rose-500/20 transition-all duration-300 font-medium text-[9px]   disabled:opacity-50 active:scale-95 flex items-center justify-center gap-2"
                                    >
                                        <Trash2 size={12} /> Dismantle
                                    </button>
                                ) : isMember ? (
                                    <button
                                        onClick={(e) => { e.stopPropagation(); setIsLeaving(true); }}
                                        disabled={loading}
                                        className="w-full sm:w-auto px-4 py-2 rounded-lg bg-white/5 hover:bg-rose-500/10 text-slate-400 hover:text-rose-500 border border-white/5 hover:border-rose-500/20 transition-all duration-300 font-medium text-[9px]   disabled:opacity-50 active:scale-95 flex items-center justify-center gap-2"
                                    >
                                    ...
                                    <button
                                        ...
                                        disabled={loading}
                                        className="w-full sm:w-auto px-6 py-2.5 rounded-lg bg-primary-600 hover:bg-primary-500 text-white shadow-lg shadow-primary-900/40 transition-all duration-300 font-medium text-[12px]   disabled:opacity-50 active:scale-95 border border-white/10"
                                    >
